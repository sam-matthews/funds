...
CREATE OR REPLACE FUNCTION profit() RETURNS VOID AS $$
...
BEGIN
  ...
  FOR fund IN
    SELECT fund_name fn,
    FROM r_fund fn
    JOIN r_fund_service fs     ON fn.fund_name = fs.fund_name
     AND r_service_porfolio sp ON fs.service_name = sp.service_name
    WHERE sp.portfolio_name = 'SAM'
    ORDER BY fund_name
  LOOP
    --RAISE NOTICE '---------------------';
